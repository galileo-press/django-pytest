...
apps. Also set the *TEST_RUNNER = 'django_pytest.TestRunner'* setting.
...
If you want to use a py.test plugin, you can separate django-specific options
(like *--verbosity=2*) from py.test options with a *--*. For example, using
pytest-coverage with django-pytest works like this::

  python manage.py test -v2 -- --cover=amazonreviews --cover-report=html
